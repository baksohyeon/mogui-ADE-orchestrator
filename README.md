...
- **Model identity from transcripts, never status lines.** `scripts/model-identity-probe` samples recent assistant turns; `scripts/model-drift-audit` walks the whole transcript for transitions. A TUI status line is a renderer drawing, not a measurement (`master-ops/docs/MASTER-OPERATIONS.md` §4, §8).
