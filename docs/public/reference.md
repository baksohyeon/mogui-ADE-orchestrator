...
| `scripts/master-bootstrap-live` | `master-bootstrap-live` | Emit the live session-start bootstrap block from a handoff directory and optional role-state file. | `--handoff-dir`, `--role-state-file`, `--budget`, `--bd`. |
