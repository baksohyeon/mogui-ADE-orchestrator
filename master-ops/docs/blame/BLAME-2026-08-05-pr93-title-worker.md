...
[관측] 방금 `docs/runbooks/contract-conventions.md`를 열어 제목 절을 확인했고, 그 절은 `Titles follow conventional commits: <type>: <lowercase description>`라고 명시한다.
...
## 5. 재발방지
트리거: PR 생성 또는 제목 수정. 볼 신호: 최근 병합 PR 제목 10개와 `docs/runbooks/contract-conventions.md` 제목 절. 실패 시 중단: 제목을 쓰지 말고 `gh pr create` 또는 `gh pr edit --title` 실행 전 오케스트레이션으로 확인 질문을 보낸다.
