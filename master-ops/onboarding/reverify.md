...
8. **Template currency**: when an ADE/template path is known, run `scripts/template-check --ops "{{OPS_REPO}}" --template "{{RUNTIME_ROOT}}/master-ops"` (or the same script from the template side if the install is too old to carry it) so the report_set is `template-compare`. If no template path is available, run `--ops` alone and record that the report_set is `install-manifest` only (shape against the installed manifest, not upstream currency). Record installed version (or undeterminable), absent required paths, and unknown present paths. **Report only** — do not apply, do not write. A behind or missing-manifest result is a signal to run Upgrade mode, not a reverify write.
