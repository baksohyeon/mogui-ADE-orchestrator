...
8. **Template currency**: run `scripts/template-check --ops "{{OPS_REPO}}"` when the script is present (or from the ADE template with `--template` when the install is too old to carry it). Record the report_set name, installed version (or undeterminable), absent required paths, and unknown present paths. **Report only** — do not apply, do not write. A behind or missing-manifest result is a signal to run Upgrade mode, not a reverify write.
