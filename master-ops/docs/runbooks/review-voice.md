...
6. **Run the anti-slop self-check before sending.** Public prose is the one surface where the workspace's writing is judged by people who did not watch it being produced. On the authoring instance the check is [anti-slop](../../skills/anti-slop/SKILL.md). Keep the outgoing-prose hygiene in [Boot, Hooks, and Observability](../charter/08-boot-hooks-observability.md) (grep organization identifiers before posting), because repository scanners do not read forge prose.
