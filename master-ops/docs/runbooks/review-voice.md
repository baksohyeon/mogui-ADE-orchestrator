...
6. **Run the anti-slop self-check before sending** (`skills/anti-slop`). Public prose is the one surface where the workspace's writing is judged by people who did not watch it being produced. **This template does not ship `skills/anti-slop`.** Promoting that skill is a separate backlog item (already tracked for template inclusion). Do not treat the path as present, and do not drop the rule. Until the skill lands, use this file's rules as the voice check, and use the outgoing-prose hygiene in `docs/charter/08-boot-hooks-observability.md` (grep organization identifiers before posting) as the interim mechanical pass on forge text the repository scanners never see.
