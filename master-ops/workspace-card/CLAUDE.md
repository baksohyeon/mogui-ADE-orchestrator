...
Product repositories (`{{REPO_LIST}}`) are **worker territory**: write a contract under `{{OPS_REPO}}/contracts/`, then dispatch through the supervised path in [Dispatch Gate]({{OPS_REPO}}/docs/charter/05-dispatch-gate.md), then verify independently. The Stage 1 skeleton does not ship a local dispatch wrapper; if this installation has added one, use that wrapper. Either way the gate owns policy — it supplies the tier policy, the contract hash, and the ledger registration itself, so pass it the contract and the worktree and nothing about policy. Remote origin is canonical; every local checkout is a copy.
