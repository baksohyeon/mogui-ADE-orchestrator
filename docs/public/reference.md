...
| `scripts/workspace-descriptor-check` | `workspace-descriptor-check` | Ask whether a workspace-descriptor prohibition applies to a repository path and action (for example `direct-main-commit` or `force-push`). | `--path`, `--action`, `--config`, `--allow-unknown-repo`, `--json`. Exit 0 allowed, 1 prohibited, 2 unconfigured or invalid. Resolution: env `WORKSPACE_DESCRIPTOR` / `MOGUI_WORKSPACE_DESCRIPTOR` → `config/workspace-descriptor.json` → honest unconfigured. |
