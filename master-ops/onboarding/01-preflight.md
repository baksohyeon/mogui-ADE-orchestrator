...
Where we are: the Herald has explained the map, and nothing on the machine has been changed yet. What we decide next: whether this machine has the tools needed to raise the Master safely. Explain gently that the first no-flag preflight is read-only: we are looking at doors and keys, not moving furniture. Ask whether the owner is ready for local read-only checks, which program they expect to use to run the Master (for example `claude`), and whether they approve installing the known dependency set on this machine if the checks find missing tools with known install commands. If a check fails, say plainly that we fix it together before moving on; nothing is skipped silently, because a missing tool would make the later summoning look mysterious.
...
Use `bash scripts/onboarding-preflight.sh --fix` only after the owner approves installing the known dependency set. It may install Orca, install `ctx`, install `gitleaks`, set up the `ctx` index, and add or refresh the global Orca skills. Without that approval, run the no-flag form, print the commands, and continue only where the resulting consequences allow.
