...
7. **Workspace session card**: both deployed root cards match their ops canonicals — `cmp "{{WORKSPACE_ROOT}}/CLAUDE.md" "{{OPS_REPO}}/workspace-card/CLAUDE.md" && cmp "{{WORKSPACE_ROOT}}/AGENTS.md" "{{OPS_REPO}}/workspace-card/AGENTS.md"` is silent (byte match). These are the workspace-root session cards the agent host reads, **not** the ops repository's own agent instruction pair (`{{OPS_REPO}}/CLAUDE.md` / `AGENTS.md`) and **not** the owner-facing operating card from step 10. If either root file is missing or differs, **report drift only** — do not redeploy, do not overwrite either side. Bring the drift to the living master as a rescue task.
