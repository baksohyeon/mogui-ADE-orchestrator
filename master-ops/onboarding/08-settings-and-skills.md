...
Where we are: the Master's ground rules are saved, and the Herald now checks which host-side safeguards should be prepared before the Master rises. What we decide next: who owns the host settings and security-sensitive configuration, and which safety hooks to turn on. Explain in simple terms that hooks are small host-side actions: some prepare the Master's context when it starts or resumes, some refresh it during a session, and some warn before risky actions. Ask which hosts run the Master, who owns hooks/security-sensitive configuration, and whether dispatch warning, role-state injection, and compaction probe hooks should be enabled. Present the hook wiring spec from `docs/MASTER-OPERATIONS.md` in agent notes, not as a wall of text at the owner; delegate auth, permission, secrets, credentials, and production-data work to a dedicated security/operations session.
