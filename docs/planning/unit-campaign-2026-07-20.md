...
> Status: Approved (Dorito, 2026-07-20 브레인스토밍 경유). 계획 v1(FROZEN 아키텍처의 분해)의 잔여 유닛 구현 캠페인. 이 문서는 계획 v1에 아무것도 추가하지 않는다 — 순서와 계약 골자만 확정한다.
> 배경: 같은 날 이중 Master 인스턴스 사고(ops-planning 런 로그 R+12) — U8·U9 미구현이 구조적 원인으로 실증됨.
