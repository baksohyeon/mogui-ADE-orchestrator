...
| --- | --- | --- | --- |
| Execution environment | Orca terminal sessions, `scripts/` entry points, per-repository git worktrees through Orca | Observed | The unit suite covers the entry points. Worktree creation and terminal placement are host behavior, not code in this repository. |
...
| Delegation | `dispatch_gate.py` check/register, worker contract file, JSONL gate ledger | Observed | Gate decisions and ledger writes are observable after the fact. The launch itself is not wrapped: the typed adapter dispatch path was removed, so the master starts workers on the runtime directly and the gate brackets that step. Whether *every* worker-creation path in a given workspace goes through the gate is Unknown from this repository alone; that is a workspace wiring property. |
...
Read next: [Orca Concepts](orca-concepts.md) for the host object model, then [Delegation and Review](delegation-and-review.md) and [Master Lifecycle](master-lifecycle.md). See [Reference](reference.md) for the local script entry points, and [Overview](overview.md#related-work) for how this compares to in-process agent harnesses.
