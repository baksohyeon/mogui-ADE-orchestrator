...
- **Dispatch gate, tier × fan-out, ledgered decisions.** `scripts/dispatch-gate` with `master-ops/model-tier-policy.json` caps agents per tier over a rolling window (the cost incident was ten workers on a top tier at once, and the same count one at a time). Each decision is appended to a JSONL ledger with the policy path and its `sha256`. See `master-ops/docs/MASTER-OPERATIONS.md` §4 and §8.
- **Model identity from transcripts, never status lines.** `scripts/model-identity-probe` samples recent assistant turns; `scripts/model-drift-audit` walks the whole transcript for transitions. A TUI status line is a renderer drawing, not a measurement (`MASTER-OPERATIONS.md` §4, §8).
...
- **Revival checks for frozen sessions.** Retirement is complete only when process, pane, and tty are measured gone; boot scans lineage session ids for revived processes (`succession-boot-card.md`). Paid for by four retired masters revived at once from a mobile resume.
