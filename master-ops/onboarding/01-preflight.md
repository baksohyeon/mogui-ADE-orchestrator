...
Where we are: the Herald has explained the map, and nothing on the machine has been changed yet. What we decide next: whether this machine has the tools needed to raise the Master safely. Explain gently that the first no-flag preflight is read-only: we are looking at doors and keys, not moving furniture. Ask whether the owner is ready for local read-only checks, which program they expect to use to run the Master (for example `claude`), and whether they approve installing the known dependency set on this machine if the checks find missing tools with known install commands. Name the `ctx` side effect before asking: the installer sets up `ctx` and creates or updates a searchable local history index. If a check fails, say plainly that we fix it together before moving on; nothing is skipped silently, because a missing tool would make the later summoning look mysterious.
...
Use `bash scripts/onboarding-preflight.sh --fix` only after the owner approves installing the known dependency set and the `ctx` local history indexing that install performs. It may install Orca, install `ctx`, install `gitleaks`, set up the `ctx` index, and add or refresh the global Orca skills. Without that approval, run the no-flag form, print the commands, and continue only where the resulting consequences allow.
...
The dispatch gate matches worker model strength to task weight and caps any tiers the owner chooses to cap. Top-tier use is now an owner-approval question in `master-ops/scripts/dispatch`, not a fan-out cap in the policy file. That only works when the tier file lists models this machine actually has; a hardcoded template list cannot ship as the live policy.

**Owner ask (warm ELI5, yes/no):** May I check which agent programs are installed on this machine, and which model names they advertise when a CLI exposes them? I use that list for two reasons only: to match each dispatched task to a strong-enough model instead of always picking the most expensive one, and to know when a top-tier dispatch must ask you for approval. You can say no; then we fill the list with only the agents and models you name by hand, and any unlisted model resolves to the `unknown` tier (capped only when `fanout_caps.unknown` is set; otherwise uncapped like any tier with no cap entry).
...
- **Consent no:** do not probe. Ask the owner to name the runtimes and model ids they want listed; write only those entries; set `consent: "no"`. Empty `tiers` lists are valid — unlisted models still resolve as `unknown`.
...
# fill version 2 shape: agents[], tiers, fanout_caps (per-tier; missing key = uncapped), window_seconds, consent
```

Schema (gate-consumed fields): `version` must be `2`; `tiers` maps tier name to model-id lists; `fanout_caps` maps tier name to a non-negative integer cap (a missing key, including for `top` and `unknown`, means uncapped); `window_seconds` is the rolling window. Extra keys (`agents`, `consent`, `_docs`) are documentation for the install and are ignored by the gate parser. Resolution for the gate: environment `DISPATCH_TIER_POLICY` → `config/model-tier-policy.json` when present → template `master-ops/model-tier-policy.json`. Owner directive 2026-08-05 moved top-tier control to `master-ops/scripts/dispatch --top-approved`.
...
- the agent-inventory consent question was asked with both purposes stated; `config/model-tier-policy.json` exists (instance-owned, not committed) with `version: 2`, optional per-tier `fanout_caps`, and either measured `agents`/`tiers` (consent yes) or only owner-named entries (consent no); no model id was guessed; the template example was not replaced in git
