...
out=$(env -u BEADS_DIR MOGUI_SETTINGS_FILE="$TMP/settings.json" MOGUI_HOOKS_DIR="$TMP/hooks" WORKSPACE_ROOT="$TMP" bash "$T" 2>/dev/null | grep '^\[protections\]')
...
out2=$(env -u BEADS_DIR MOGUI_SETTINGS_FILE="$TMP/settings2.json" MOGUI_HOOKS_DIR="$TMP/hooks" WORKSPACE_ROOT="$TMP" bash "$T" 2>/dev/null | grep '^\[protections\]')
...
mout=$(env -u BEADS_DIR MOGUI_SETTINGS_FILE="$TMP/settings.json" MOGUI_HOOKS_DIR="$TMP/hooks" WORKSPACE_ROOT="$TMP" bash "$TMP/mutant.sh" 2>/dev/null | grep '^\[protections\]')
