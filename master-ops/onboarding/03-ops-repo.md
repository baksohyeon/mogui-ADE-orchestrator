...
If new or empty, copy the Stage 1 skeleton from `{{RUNTIME_ROOT}}/master-ops/` using the paths listed in `{{RUNTIME_ROOT}}/master-ops/MANIFEST.json` (generated by `scripts/generate-manifest`; the exclusion list lives in that generator — `TEMPLATE-VERSION`, `CHANGELOG.md`, `ONBOARDING.md`, and the `onboarding/` directory stay on the template side). The committed `MANIFEST.json` itself is installed and carries the template version stamp. If the ops repository already exists, stop Founding: when the tree needs the template layer brought forward, route to Upgrade (`upgrade.md`); when the master is dead, half-finished, or succession is the question, stop and use the ops repository's `docs/runbooks/succession-boot-card.md` instead of re-copying or re-founding. Do not push unless explicitly asked. The skeleton includes `workspace-card/` (canonical workspace-root session card); that directory is in the manifest and is not excluded.
...
- `MANIFEST.json` is present and its `template_version` matches the template stamp used for this install
