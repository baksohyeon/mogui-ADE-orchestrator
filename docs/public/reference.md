...
| `scripts/master-succeed` | `master-succeed spawn` | Spawn or dry-run a clean successor terminal for a selected workspace. | `--workspace-selector`, `--expected-placement`, `--kickoff-text` or `--kickoff-file`, `--root`, `--model`, `--title`, `--dry-run`, `--json`. |
