...
6. **Run the anti-slop self-check before sending.** Public prose is the one surface where the workspace's writing is judged by people who did not watch it being produced. On the authoring instance the check is the anti-slop skill (SKILL.md under the anti-slop skill directory). **This template does not ship that skill**, so there is no existing file to open with a markdown link from here; promoting it is a separate backlog item already tracked for template inclusion. Do not invent a live link to a missing file, and do not drop the rule. Until the skill lands, use this file's rules as the voice check, and use the outgoing-prose hygiene in [Boot, Hooks, and Observability](../charter/08-boot-hooks-observability.md) (grep organization identifiers before posting) as the interim mechanical pass on forge text the repository scanners never see.
