...
Tier policy file selection: explicit `--tier-policy` on the CLI, then environment override named `DISPATCH_TIER_POLICY`, then instance `config/model-tier-policy.json` (written at onboarding after agent-inventory consent), then template `model-tier-policy.json`.

The gate enforces that policy file. A version 2 policy gates on tier multiplied by fan-out rather than on identity, because the incident it exists for was a top-tier spread across ten workers: each named tier may carry a cap on agents dispatched inside a window; a missing `fanout_caps` entry means uncapped for that tier, including `top` and `unknown`. When the policy includes a `runtimes` list, the gate checks `runtime` against that allow-list and denies unknown values; when the key is absent, runtime behavior stays as before for compatibility. This runtime check is not routing judgment: a green gate means only that runtime is known, not that it is the right family for the work, which remains charter §4 coordinator judgment. An unlisted model falls in the `unknown` tier rather than being denied, and is not treated as if it were frontier/`top` merely because the id is absent from every list. Exceeding a stated cap requires `--tier-override "<reason>"`. The window counts accumulation, not concurrency, so ten sequential single-agent dispatches reach the same cap as one fan-out of ten when that tier still has a cap; an override passes one request without refunding what the window already counted. Version 1 policies keep their identity-based behaviour unchanged. Model identifiers match casefolded, so a tier spelled in another case is the same tier. Each decision records the policy path, its `sha256`, and the tier that decided, because that path is caller-supplied; `dispatch-gate report` lists every policy a span was judged against and every tier it spent agents in, including `unknown`, and more than one policy row means the span was not judged against one policy.

Owner directive 2026-08-05: the top-tier fanout cap is removed. The template `model-tier-policy.json` no longer ships `fanout_caps.top`; the supervised dispatch wrapper refuses a top-tier model unless the caller supplies the owner-approved reason with `--top-approved "<reason>"`. This is procedure evidence in the run log, not an authentication boundary. Register new model ids in a tier list as they appear so an absent id does not become the harshest verdict by default.

For dry runs, `dispatch-gate check --no-record` evaluates the current ledger without appending a decision or issuing a ticket. Use it for `scripts/dispatch --check-only`, because a dry run that writes a row spends the same fanout budget it exists to inspect.
...
The `--probe-cmd` must exit 0 and print the job id in stdout so the gate can verify it. A probe that prints only a filename containing the job id can satisfy both predicates without proving that the artifact contents were read, so it delivers a verification stamp without verification. `grep -l` itself reads file contents before printing matching filenames; it is not a filename-only match, but it is still unsuitable when its filename output is the only evidence. Use probe commands that output file contents, like `grep <job-id> logfile` or `cat evidence.txt`, rather than file-listing output.
...
The supervised dispatch command is vendor-neutral Orca orchestration: bind a Run, create the Task with `orca orchestration task-create`, attach the worker with `orca orchestration worker-start` (or `dispatch --inject`), then wait event-driven with `check --wait --types worker_done,escalation,question`. A coordinator clears each delivery by chaining that response's camelCase `deliveryId` into the next wait's acknowledgement flag before waiting again; an unacknowledged backlog makes `--wait` return the same stale batch forever instead of blocking for new work. Cross-master traffic addresses a standing Run rather than a transient handle. Raw terminal polling and vendor-direct CLIs bypass task and Dispatch provenance and `worker_done` authority and are non-compliant dispatch paths. Vendor plugins are allowed for non-dispatch uses such as second-opinion review. Record accidental work outside orchestration plainly as non-orchestrated; never relabel it orchestrated. (charter rule since template v5)
